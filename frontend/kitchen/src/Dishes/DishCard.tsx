...
export default function DishCard({ doc, editorHandleRef, user }: { doc: DocumentSnapshot<DocumentData>, editorHandleRef: MutableRefObject<EditorHandle | undefined>, user: User }) {
    ...
    return (
        <Grid item xs={12} sm={6} lg={3} md={4}>
            <Card>
                <DishCardHeader dish={dish} menuActions={[
                    {
                        name: 'Edit',
                        action: () => editorHandleRef.current?.edit(dish, doc.ref, url, setUrl)
                    },
                    {
                        name: 'Delete',
...
export function NoImage() {
    return (
        <>
            ...
            <Typography variant="caption" color={theme => theme.palette.grey[400]}>no image</Typography>
        </>
    )
}

function DishCardHeader({ menuActions, dish }: {menuActions: {name: string, action: () => void}[], dish: Dish}) {
    ...
    return (
        <>
            ...
            <CardHeader title={dish.name} titleTypographyProps={{variant: 'subtitle1'}} subheader={getDateStr(dish.lastMade)} action={
...
function DishPicture({ url }: {url: string | null | undefined | FirebaseError}) {
